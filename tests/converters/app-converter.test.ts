...
describe('Stack Converter', () => {
    ...
    test('can convert multiple', async () => {
        ...
        const cidrBlock = await promiseOf(subnet.ipv6CidrBlock);
        expect(cidrBlock).toEqual('cidr_ipv6AddressAttribute');
        const subnet2 = converter.resources.get({ stackPath: manifest.id, id: 'other2' })
            ?.resource as native.ec2.Subnet;
    ...
    test('can convert with custom resources', async () => {
        ...
        const customResource = converter.resources.get({
            stackPath: manifest.id,
        ...
        const customResourceRole = converter.resources.get({
            stackPath: manifest.id,
    ...
    test('can convert nested stacks', async () => {
        ...
        const rootBucket = converter.resources.get({ stackPath: manifest.id, id: 'bucket' })
            ?.resource as native.s3.Bucket;
        ...
        const nestedStackResource = converter.resources.get({
            stackPath: manifest.id,
        ...
        const nestedBucket = converter.resources.get({ stackPath: `${manifest.id}/nesty`, id: 'bucket43879C71' })
            ?.resource as native.s3.Bucket;
        ...
        const policy = converter.resources.get({
            stackPath: manifest.id,
        ...
        expect(policyDocument.Statement[1].Resource).toEqual(
            expect.arrayContaining(['bucket43879c71_arn', 'bucket43879c71_arn/*']),
        );
    });
    ...
    describe('asOutputValue', () => {
        ...
        test('throws if token is found in multiple stacks', async () => {
            ...
            converter.convert(new Set());
            expect(() => converter.asOutputValue('DUMMY')).toThrow(
                '[CDK Adapter] Value found in multiple stacks: stack/nested and stack/otherNested. Pulumi cannot resolve this value.',
            );
        });
    });
});
