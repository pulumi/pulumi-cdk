...
function testSub(cases: SubCase[]) {
    for (const c of cases) {
        ...
        expect(parts).toMatchObject(c.expected);
    }
}

describe('Sub tests', () => {
    test('Correctly parses Fn::Sub with a single hole', () => {
        testSub([
    ...
    test('Correctly parses Fn::Sub with no holes', () => {
        testSub([
    ...
    test('Correctly parses Fn::Sub with literal holes', () => {
        testSub([
    ...
    test('Correctly parses Fn::Sub with multiple holes', () => {
        testSub([
            {
            ...
            {
                template: 'Foo ${Foo.Attr} Bar ${Bar.Attr} Baz',
                expected: [
    ...
    test('Correctly parses regression cases', () => {
        testSub([
            {
                template: 'cdk-hnb659fds-assets-${AWS::AccountId}-${AWS::Region}',
                expected: [
