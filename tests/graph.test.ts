...
function testGraph(fn: (scope: Construct) => void, expected: (string | RegExp)[], done: any) {
    ...
    expect(sortedPaths.length).toEqual(expected.length);
...
describe('Graph tests', () => {
    test('Test sort for single resource', (done) => {
        testGraph(
            (stack) => {
                new s3.Bucket(stack, 'MyFirstBucket', { versioned: true });
            },
            ['graphtest', 'graphtest/MyFirstBucket', 'graphtest/MyFirstBucket/Resource'],
            done,
        );
    });

    test('Test sort for ALB example', (done) => {
        testGraph(
            (stack) => {
    ...
    test('Test sort for appsvc example', (done) => {
        testGraph(
            (stack) => {
    ...
    test('Test sort for apprunner example', (done) => {
        testGraph(
            (stack) => {
            ...
            ['graphtest', 'graphtest/service', 'graphtest/service/Resource'],
            done,
        );
    });
});
