...
- Important gotcha: The CLI runs against the compiled files in `node_modules/@pulumi/cdk-convert-core`, even during development. After changing any TS under `packages/cdk-convert-core/src`, rerun the workspace build (e.g. `npm run build` or `tsc -b packages/cdk-convert-core/tsconfig.json`) so the linked package emits fresh JS before invoking CLI commands (e.g. `npx ts-node src/cli/cli-runner.ts`).
